...
PATIENCE = 10
DEFAULT_EPOCHS = 1000
def build_LSTM(model_name,input_shape,GLOBALMAXPOOL1D):
    ...
    #model.add(Conv2D(kernels, (kernel_rows,kernel_c ), strides=(1, 1), kernel_regularizer=regularizer, name='conv0'))
    #relu
    if(GLOBALMAXPOOL1D):
...
def main(argv):
    ...
    if args.train is not None:
        ...
        for full_path in subfolders:
            ...
            if(args.gmaxpool1d):
                model_name = dataset_name + "-LSTM-GMAXP1D"
            else:
                model_name = dataset_name + "-LSTM"
            batch_size=1024
            best_model=build_LSTM(model_name,input_shape=(X_train.shape[1],X_train.shape[2]),GLOBALMAXPOOL1D=args.gmaxpool1d)
